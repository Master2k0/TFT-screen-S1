...
function collect(message) {
    ...
    if (_collect_count < TIMEOUT_COUNT) {
        ...
        return network_usage(message.iface).then(results => {
            ...
            const _current_rx_bytes = Number(results[1]);
            const _current_tx_bytes = Number(results[2]);

            const _current_rx_packets = Number(results[3]);
            const _current_tx_packets = Number(results[4]);
            ...
            if (!_fault) {
                logger.error(message.iface + ' network_thread: network stats read error: ' + err);
                _fault = true;
            }         
        });
    }
